...
namespace rage
{
	...
	inline joaat_t rockstar_crc_hash(std::string_view str, uint32_t key = 0x04C11DB7)
	{
		int hash = 0;
	...
	inline bool reverse_joaat(const char* key, uint32_t hash)
	{
	...
	inline std::string joaat_to_string(const char* key, uint32_t hash)
	{
		hash *= 0x3FFF8001;  // inverse of hash += hash << 15;
		hash ^= (hash >> 11) ^ (hash >> 22);
		hash *= 0x38E38E39;  // inverse of hash += hash << 3;
